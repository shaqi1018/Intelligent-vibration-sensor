...
int Mic_Start(uint32_t sample_rate_hz, uint16_t gain_db)
{
  ...
  /* I2C2 与 PCF85063 RTC 共享总线：序列级加锁，整段 ES8311 配置不与 RTC 读交错(M1)。
   * 锁不覆盖上面的 osDelay(50)，只覆盖真正的 I2C2 总线事务；每条返回路径各解锁一次。 */
  AppI2c2Lock();
  if (ES8311_Probe() != 0) { AppI2c2Unlock(); return -1; }
  if (ES8311_InitAdc(sample_rate_hz, gain_db) != 0) { AppI2c2Unlock(); return -2; }
  if (MX_SAI1_Init(sample_rate_hz) != HAL_OK) { ES8311_PowerDown(); AppI2c2Unlock(); return -3; }
  s_dropped = 0U;
  ...
  if (HAL_SAI_Receive_DMA(&hsai_BlockA1, (uint8_t *)s_dma_buf,
    ...
    MX_SAI1_DeInit(); ES8311_PowerDown(); AppI2c2Unlock(); return -4;
  }
  AppI2c2Unlock();
  return 0;
}
