...
#include "usb_otg.h"
#include "app_acq.h"
/* user_ctrl removed in path/usb — no button/battery */
#include "board_io.h"
#include "app_time.h"
#include "app_locks.h"
...
#endif
  /* UserCtrl task removed in path/usb — no button/battery/power-off */
...
void StartLoggerTask(void *argument)
{
  /* path/usb: SD recording removed. Data flows directly from sensors to USB via
   * UsbWcidApp_SendCsv (LSM/H3/QMA/MIC). Ring buffers remain allocated (保持无害缓冲),
   * but are not consumed here. Logger task kept as idle shell so the handle/diag
   * stays valid; stack shrunk to minimal since it does nothing but sleep. */
  (void)argument;
  printf("[Logger] task idle (path/usb — USB streaming only, no SD writes)\r\n");
  for (;;) { osDelay(10000U); }
}
