...
extern void UX_App_PCD_ISOINIncompleteCallback(PCD_HandleTypeDef *hpcd, uint8_t epnum);
extern void UX_App_PCD_ISOOUTIncompleteCallback(PCD_HandleTypeDef *hpcd, uint8_t epnum);
...
{
  if (IS_MSC()) { (void)USBD_LL_IsoINIncomplete(hpcd->pData, epnum); }
  else          { UX_App_PCD_ISOINIncompleteCallback(hpcd, epnum); }
}

void HAL_PCD_ISOOUTIncompleteCallback(PCD_HandleTypeDef *hpcd, uint8_t epnum)
{
  if (IS_MSC()) { (void)USBD_LL_IsoOUTIncomplete(hpcd->pData, epnum); }
  else          { UX_App_PCD_ISOOUTIncompleteCallback(hpcd, epnum); }
}
