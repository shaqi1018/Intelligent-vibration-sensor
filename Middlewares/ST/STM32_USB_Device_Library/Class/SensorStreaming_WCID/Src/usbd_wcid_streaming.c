...
static uint8_t  USBD_WCID_STREAMING_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
  ...
  else
  {
    ...
    ((USBD_WCID_STREAMING_ItfTypeDef *)pdev->pUserData[0])->Init();
    /* Init Xfer states */
    hwcid->RxState = 0;
...
static uint8_t  USBD_WCID_STREAMING_DeInit(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
  ...
  if (pdev->pClassData != NULL)
  {
    ((USBD_WCID_STREAMING_ItfTypeDef *)pdev->pUserData[0])->DeInit();
  ...
  switch (req->bmRequest & USB_REQ_TYPE_MASK)
  {
    case USB_REQ_TYPE_CLASS :
    {
      if (req->wLength != 0U)
      {
        ...
        {
          ((USBD_WCID_STREAMING_ItfTypeDef *)pdev->pUserData[0])->Control(0, req->bRequest, req->wValue, req->wIndex,
                                                                       (uint8_t *)hwcid->data,
                                                                       req->wLength);
...
{
  USBD_WCID_STREAMING_HandleTypeDef   *hwcid = (USBD_WCID_STREAMING_HandleTypeDef *) pdev->pClassData;
  if ((pdev->pUserData[0] != NULL) && (hwcid->CmdOpCode != 0xFFU))
  {
    ((USBD_WCID_STREAMING_ItfTypeDef *)pdev->pUserData[0])->Control(1, hwcid->CmdOpCode, hwcid->wValue,  hwcid->wIndex,
                                                                 (uint8_t *)hwcid->data,
                                                                 hwcid->CmdLength);
...
static USBD_StatusTypeDef  USBD_VendDevReq(USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef *req)
{
  ...
  switch (req->wIndex)
  {
    case USB_DESC_TYPE_OS_FEATURE_EXT_COMPAT_ID:      /* wIndex=4: Windows requests CompatID → return WINUSB */
    {
    ...
    case USB_DESC_TYPE_OS_FEATURE_EXT_PROPERTIES:     /* wIndex=5: Windows requests Properties → return GUID */
    {
