...
/*               ST Microelectronics STM32U5G9J-DK2 Flash             */
/*                                                                     */
/***********************************************************************/
...
{
  if (Init_OSPI() != LOADER_STATUS_SUCCESS)
    return 1;
  else
    return 0;
}
...
int EraseChip(void)
{
  if (MassErase() != LOADER_STATUS_SUCCESS)
    return 1;
  else
    return 0;
}
...
int EraseSector(unsigned long adr)
{
  ...
  if (SectorErase((uint32_t)block_start & 0xffffffff, ((uint32_t)block_start & 0xffffffff) + block_size) != LOADER_STATUS_SUCCESS)
    return 1;
  else
    return 0;
}
...
{
  if (Write(block_start, size, buffer) != LOADER_STATUS_SUCCESS)
    return 1;
  else
    return 0;
}
